...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    static const char page[] =
        ...
        ".mach{display:grid;gap:10px;margin-top:10px}.mach.empty{color:#9fb0bf}.macrow{display:grid;grid-template-columns:26px 58px minmax(142px,max-content) minmax(120px,1fr) 88px;align-items:center;gap:10px;min-height:44px;border:1px solid #33404b;border-radius:8px;padding:8px;background:#182029}.macrow strong{text-align:right}.macrow small{color:#9fb0bf;text-align:right}.macsrc{width:18px;height:18px;justify-self:center}.maclabel{color:#9fb0bf;font:12px Consolas,monospace}.maclabel span{white-space:nowrap}.mactrack{height:16px;overflow:hidden;border-radius:4px;background:#0d1116}.macbar{height:100%;border-radius:inherit;background:#62a8ff}.srcdiag{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-top:12px}.srcdiag div{border:1px solid #33404b;border-radius:8px;padding:10px;background:#182029}.srcdiag span{display:block;color:#9fb0bf;font-size:12px;text-transform:uppercase}.srcdiag b{display:block;margin-top:4px;overflow-wrap:anywhere}"
        ...
        "function normmac(v){v=String(v||'').trim().toUpperCase().replaceAll('-',':');return /^[0-9A-F]{2}(:[0-9A-F]{2}){5}$/.test(v)?v:''}"
        "async function setSourceMac(mac,on){const m=normmac(mac);if(on&&!m)return;const body={csi_source_mac:on?m:'',csi_source_filter_enabled:!!on};cfg=await(await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})).json();calmsg.textContent=on?'CSI source set to '+m:'CSI source filter disabled';msgUntil=Date.now()+3000}"
        ...
        "function renderMach(s){const rows=Array.isArray(s.csi_mac_histogram)?s.csi_mac_histogram.filter(x=>x&&x.mac).sort((a,b)=>(b.count||0)-(a.count||0)):[];renderSourceDiag(s);if(!rows.length){mach.className='mach empty';mach.textContent='No CSI MACs observed yet.';return}const max=Math.max(...rows.map(x=>+x.count||0),1),active=s.csi_source_filter_enabled?normmac(s.csi_source_mac||(s.csi_source_mac_diagnostics||{}).mac):'';mach.className='mach';mach.innerHTML=rows.map(x=>{const m=normmac(x.mac),ch=m&&m==active?' checked':'';return '<div class=macrow><input class=macsrc type=checkbox data-mac=\"'+String(x.mac)+'\"'+ch+'><strong>'+Number(x.count||0)+'</strong><div class=maclabel><span>'+String(x.mac)+'</span></div><div class=mactrack><div class=macbar style=\"width:'+Math.max(4,((+x.count||0)/max)*100)+'%\"></div></div><small>'+String(x.last_seen_ms==null?'n/a':x.last_seen_ms)+' ms ago</small></div>'}).join('');mach.querySelectorAll('.macsrc').forEach(e=>e.onchange=()=>setSourceMac(e.dataset.mac,e.checked))}"
