...
Version `0.5.3` is still a prototype, but the foundation now includes the
...
rows, direct graph pan/pinch controls, inactive-receiver exclusion from Pi-side
history/trigger logic, and a separate ESP32-S3-WROOM-1U receiver build for the
higher-capability receiver boards. Full app login enforcement and polished alarm
arming workflows remain future hardening work.
