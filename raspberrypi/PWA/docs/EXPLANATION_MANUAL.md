...
From version `0.2.1`, the node settings panel can also read and save an ESP32
node's persisted stillness calibration baseline. That baseline lives on the
ESP32 in NVS, so a calibrated node can recover after power loss without
automatically recalibrating while someone might be moving nearby.
